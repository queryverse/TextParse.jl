@inline function eatwhitespaces(str::Union{MyStringType, String}, i=1, len=lastindex(str))
    while i<=len
...
@inline function eatnewlines(str::Union{MyStringType, String}, i=1, len=lastindex(str))
    count = 0
    while i<=len
        @inbounds b = codeunit(str, i)
        if b == 0xd # '\r'
...
@inline function tryparsenext_base10_digit(T,str::Union{MyStringType, String},i, len)
    i > len && @goto error
    @inbounds b = codeunit(str,i)
    diff = b-0x30
...
@inline function parse_uint_and_stop(str::Union{MyStringType, String}, i, len, n::T) where {T <: Integer}
    ten = T(10)
...
@inline function read_digits(str::Union{MyStringType, String}, i, len)
...
@inline function _is_e(str::Union{MyStringType, String}, i)
    @inbounds b = codeunit(str, i)
    return  (b==0x65) | (b==0x45)
end

@inline function _is_negative(str::Union{MyStringType, String}, i)
    @inbounds b = codeunit(str, i)
    return b==0x2d
end

@inline function _is_positive(str::Union{MyStringType, String}, i)
    @inbounds b = codeunit(str, i)
    return b==0x2b
end

@inline function tryparsenext(::Numeric{F}, str::Union{MyStringType, String}, i, len) where {F<:AbstractFloat}
    R = Nullable{F}
